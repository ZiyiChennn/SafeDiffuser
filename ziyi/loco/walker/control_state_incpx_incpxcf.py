...
for i in range(len(milestone_indices) - 1):
    ...
    if len(segment) > 0:
        ...
        weighted_sum = segment_full[:, 0] + 0.1 * segment_full[:, 9]
...
if len(last_segment) > 0:
    ...
    weighted_sum = last_segment_full[:, 0] + 0.1 * last_segment_full[:, 9]
    max_val = np.max(weighted_sum)
    max_val_relative_index = np.where(weighted_sum == max_val)[0][0]
...
for i in range(len(milestone_numbers) - 1):
    ...
    # 使用数组切片进行最终计算
    final_value = 1.35 - results[i, 0] - 0.1 * results[i, 9]
...
final_value = 1.35 - results[-1, 0] - 0.1 * results[-1, 9]
