...
for i in range(len(milestone_indices) - 1):
    ...
    if len(segment) > 0:
        ...
        segment_full = full_array[start_index:end_index, :]
        weighted_sum = segment_full[:, 0] + 0.1 * segment_full[:, 6]
        max_val = np.max(weighted_sum)
        max_val_relative_index = np.where(weighted_sum == max_val)[0][0]
...
if len(last_segment) > 0:
    last_segment_full = full_array[last_start_index:, :]
    weighted_sum = last_segment_full[:, 0] + 0.1 * last_segment_full[:, 6]
    max_val = np.max(weighted_sum)
    max_val_relative_index = np.where(weighted_sum == max_val)[0][0]
...
for i in range(len(milestone_numbers) - 1):
    ...
    # 使用数组切片进行最终计算
    final_value = 1.5 - results[i, 0] - 0.1 * results[i, 6] # expert 1.6 replay 1.55 medium 1.6
...
final_value = 1.5 - results[-1, 0] - 0.1 * results[-1, 6]
